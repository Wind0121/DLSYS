...
__device__ size_t index_transform(size_t index, CudaVec shape, CudaVec strides, size_t offset) {
  int32_t idxs[MAX_VEC_SIZE];
  int32_t cur_size, pre_size = 1;
...
void __global__ ScalarSetitemKernel(scalar_t val, scalar_t* out, size_t size, CudaVec shape, 
                              CudaVec strides, size_t offset) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) {
...
void Matmul(const CudaArray& a, const CudaArray& b, CudaArray* out, uint32_t M, uint32_t N,
  ...
  /// BEGIN SOLUTION
  CudaDims dim;
  size_t block_x = 16, block_y = 16;
  size_t grid_y = (M + block_y - 1) / block_y, grid_x = (P + block_x - 1) / block_x;
  dim.block.x = block_x, dim.block.y = block_y;
  dim.grid.x = grid_x, dim.grid.y = grid_y;
...
__global__ void ReduceMaxKernel(const scalar_t* a, scalar_t* out, size_t out_size, size_t reduce_size) {
  ...
  if (gid < out_size) {
    ...
    for (int j = 0; j < reduce_size; j++) {
      max_num = max(max_num, a[gid * reduce_size + j]);
    }
    out[gid] = max_num;
  }
}
